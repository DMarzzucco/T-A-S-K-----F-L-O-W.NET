...
import { UserServiceProps } from './interface/userService.interface';

@Injectable()
export class UsersService implements UserServiceProps {

  constructor(private readonly prisma: PrismaService) { }

  public async create(body: CreateUserDto): Promise<CreateUserDto> {
  ...
  public async finBy({ key, value }: findByProps): Promise<User> {
  ...
  public async update(id: number, data: UpdateUserDto): Promise<UpdateUserDto> {
    return await this.prisma.user.update({ where: { id: id }, data: data })
  }

  public async updateToken(id: number, refreshToken: string): Promise<User> {
